...
#Screen Display------------------------------------------------------------------
LEVEL5 = Tk()
LEVEL5.title("BREAKOUT")
LEVEL5.resizable(0, 0)
canvas = Canvas(LEVEL5, bg="Black", width="1000", height="700")
canvas.pack()
LEVEL5.update()
#LEVEL--------------------------------------------------------------------------
Level = Label(LEVEL5, text="Current level: 4", bg="Black", fg="White", font=("Space Bd BT",15))
#LIFE POINTS
LP = Label(LEVEL5, text="Life points:", bg="Black", fg="White", font=("Space Bd BT",15))
#TIME----------------------------------------------------------------------------
Time = Label(LEVEL5, text="Time left:", bg="Black", fg="White", font=("Space Bd BT",15))
#LOGO#--------------------------------------------------------------------------------
Logo = Label(LEVEL5, text="BREAKOUT", bg="Black", fg="White", font=("Space Bd BT",40))
#PACKS#-------------------------------------------------------------------------------
...
Brick_Left = 70 #Number of bricks in the beginning
Time = 170 #Time
...
class Ball:
    def __init__(self, canvas, paddle,
        ...
        self.brick61  = brick61
        self.brick62  = brick62
        self.brick63  = brick63
        self.brick64  = brick64
        self.brick65  = brick65
        self.brick66  = brick66
        self.brick67  = brick67
        self.brick68  = brick68
        self.brick69  = brick69
        self.brick70  = brick70
        ...
        if pos[2] >= paddle_pos[0] and pos[0] <= paddle_pos[2]:
            if pos[3] >= paddle_pos[1] and pos[3] <= paddle_pos[3]:
                ...
                if Time == 0:
                    ...
                    LEVEL5.after(3000, lambda: LEVEL5.destroy())
    ...
    def hit_brick61(self, pos):
        brick61_pos = self.canvas.coords(self.brick61.id)
        if len(brick61_pos) != 0:
            if pos[2] >= brick61_pos[0] and pos[0] <= brick61_pos[2]:
                if pos[3] >= brick61_pos[1] and pos[1] <= brick61_pos[3]:
                    self.y = 4
                    canvas.delete(brick61.id)
                    global Brick_Left
                    Brick_Left -= 1
                    bricks()
                    if Brick_Left == 0:
    ...
    def hit_brick62(self, pos):
        brick62_pos = self.canvas.coords(self.brick62.id)
        if len(brick62_pos) != 0:
            if pos[2] >= brick62_pos[0] and pos[0] <= brick62_pos[2]:
                if pos[3] >= brick62_pos[1] and pos[1] <= brick62_pos[3]:
                    self.y = 4
                    canvas.delete(brick62.id)
                    global Brick_Left
                    Brick_Left -= 1
                    bricks()
                    if Brick_Left == 0:
    ...
    def hit_brick63(self, pos):
        brick63_pos = self.canvas.coords(self.brick63.id)
        if len(brick63_pos) != 0:
            if pos[2] >= brick63_pos[0] and pos[0] <= brick63_pos[2]:
                if pos[3] >= brick63_pos[1] and pos[1] <= brick63_pos[3]:
                    self.y = 4
                    canvas.delete(brick63.id)
                    global Brick_Left
                    Brick_Left -= 1
                    bricks()
                    if Brick_Left == 0:
    ...
    def hit_brick64(self, pos):
        brick64_pos = self.canvas.coords(self.brick64.id)
        if len(brick64_pos) != 0:
            if pos[2] >= brick64_pos[0] and pos[0] <= brick64_pos[2]:
                if pos[3] >= brick64_pos[1] and pos[1] <= brick64_pos[3]:
                    self.y = 4
                    canvas.delete(brick64.id)
                    global Brick_Left
                    Brick_Left -= 1
                    bricks()
                    if Brick_Left == 0:
    ...
    def hit_brick65(self, pos):
        brick65_pos = self.canvas.coords(self.brick65.id)
        if len(brick65_pos) != 0:
            if pos[2] >= brick65_pos[0] and pos[0] <= brick65_pos[2]:
                if pos[3] >= brick65_pos[1] and pos[1] <= brick65_pos[3]:
                    self.y = 4
                    canvas.delete(brick65.id)
                    global Brick_Left
                    Brick_Left -= 1
                    bricks()
                    if Brick_Left == 0:
    ...
    def hit_brick66(self, pos):
        brick66_pos = self.canvas.coords(self.brick66.id)
        if len(brick66_pos) != 0:
            if pos[2] >= brick66_pos[0] and pos[0] <= brick66_pos[2]:
                if pos[3] >= brick66_pos[1] and pos[1] <= brick66_pos[3]:
                    self.y = 4
                    canvas.delete(brick66.id)
                    global Brick_Left
                    Brick_Left -= 1
                    bricks()
                    if Brick_Left == 0:
    ...
    def hit_brick67(self, pos):
        brick67_pos = self.canvas.coords(self.brick67.id)
        if len(brick67_pos) != 0:
            if pos[2] >= brick67_pos[0] and pos[0] <= brick67_pos[2]:
                if pos[3] >= brick67_pos[1] and pos[1] <= brick67_pos[3]:
                    self.y = 4
                    canvas.delete(brick67.id)
                    global Brick_Left
                    Brick_Left -= 1
                    bricks()
                    if Brick_Left == 0:
    ...
    def hit_brick68(self, pos):
        brick68_pos = self.canvas.coords(self.brick68.id)
        if len(brick68_pos) != 0:
            if pos[2] >= brick68_pos[0] and pos[0] <= brick68_pos[2]:
                if pos[3] >= brick68_pos[1] and pos[1] <= brick68_pos[3]:
                    self.y = 4
                    canvas.delete(brick68.id)
                    global Brick_Left
                    Brick_Left -= 1
                    bricks()
                    if Brick_Left == 0:
    ...
    def hit_brick69(self, pos):
        brick69_pos = self.canvas.coords(self.brick69.id)
        if len(brick69_pos) != 0:
            if pos[2] >= brick69_pos[0] and pos[0] <= brick69_pos[2]:
                if pos[3] >= brick69_pos[1] and pos[1] <= brick69_pos[3]:
                    self.y = 4
                    canvas.delete(brick69.id)
                    global Brick_Left
                    Brick_Left -= 1
                    bricks()
                    if Brick_Left == 0:
    ...
    def hit_brick70(self, pos):
        brick70_pos = self.canvas.coords(self.brick70.id)
        if len(brick70_pos) != 0:
            if pos[2] >= brick70_pos[0] and pos[0] <= brick70_pos[2]:
                if pos[3] >= brick70_pos[1] and pos[1] <= brick70_pos[3]:
                    self.y = 4
                    canvas.delete(brick70.id)
                    global Brick_Left
                    Brick_Left -= 1
                    bricks()
                    if Brick_Left == 0:
    ...
    def draw(self):
        ...
        if self.hit_brick60(pos) == True:
            self.y = 4
            canvas.delete(brick60.id)
        #if the ball hits the brick61 it will bounce back
        if self.hit_brick61(pos) == True:
            self.y = 4
            canvas.delete(brick61.id)
        #if the ball hits the brick62 it will bounce back
        if self.hit_brick62(pos) == True:
            self.y = 4
            canvas.delete(brick62.id)
        #if the ball hits the brick63 it will bounce back
        if self.hit_brick63(pos) == True:
            self.y = 4
            canvas.delete(brick63.id)
        #if the ball hits the brick64 it will bounce back
        if self.hit_brick64(pos) == True:
            self.y = 4
            canvas.delete(brick64.id)
        #if the ball hits the brick65 it will bounce back
        if self.hit_brick65(pos) == True:
            self.y = 4
            canvas.delete(brick65.id)
        #if the ball hits the brick66 it will bounce back
        if self.hit_brick66(pos) == True:
            self.y = 4
            canvas.delete(brick66.id)
        #if the ball hits the brick67 it will bounce back
        if self.hit_brick67(pos) == True:
            self.y = 4
            canvas.delete(brick67.id)
        #if the ball hits the brick68 it will bounce back
        if self.hit_brick68(pos) == True:
            self.y = 4
            canvas.delete(brick68.id)
        #if the ball hits the brick69 it will bounce back
        if self.hit_brick69(pos) == True:
            self.y = 4
            canvas.delete(brick69.id)
        #if the ball hits the brick70 it will bounce back
        if self.hit_brick70(pos) == True:
            self.y = 4
            canvas.delete(brick70.id)
...
class Paddle:
    def __init__(self, canvas):
        ...
        self.id = canvas.create_rectangle(200, 700, 370, 386, fill="red")
...
class Brick61:
    def __init__(self):
        self.id = canvas.create_rectangle(0, 300, 100, 330, fill="Blue")
    def draw(self):
        pass

class Brick62:
    def __init__(self):
        self.id = canvas.create_rectangle(100, 300, 200, 330, fill="Blue")
    def draw(self):
        pass

class Brick63:
    def __init__(self):
        self.id = canvas.create_rectangle(200, 300, 300, 330, fill="Blue")
    def draw(self):
        pass

class Brick64:
    def __init__(self):
        self.id = canvas.create_rectangle(300, 300, 400, 330, fill="Blue")
    def draw(self):
        pass

class Brick65:
    def __init__(self):
        self.id = canvas.create_rectangle(400, 300, 500, 330, fill="Blue")
    def draw(self):
        pass

class Brick66:
    def __init__(self):
        self.id = canvas.create_rectangle(500, 300, 600, 330, fill="Blue")
    def draw(self):
        pass

class Brick67:
    def __init__(self):
        self.id = canvas.create_rectangle(600, 300, 700, 330, fill="Blue")
    def draw(self):
        pass

class Brick68:
    def __init__(self):
        self.id = canvas.create_rectangle(700, 300, 800, 330, fill="Blue")
    def draw(self):
        pass

class Brick69:
    def __init__(self):
        self.id = canvas.create_rectangle(800, 300, 900, 330, fill="Blue")
    def draw(self):
        pass

class Brick70:
    def __init__(self):
        self.id = canvas.create_rectangle(900, 300, 1000, 330, fill="Blue")
    def draw(self):
        pass
